<div class="py-8 min-h-screen">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
            <a href="{{ route('diary.index') }}" wire:navigate class="inline-flex items-center gap-2 text-sm font-medium text-ink-soft hover:text-primary dark:text-ink-dark-soft dark:hover:text-primary-dark transition group">
                ...
                <span>العودة للمذاكرات</span>
            </a>

            <div class="flex items-center gap-2">
                <button type="button" 
                        wire:click="$dispatch('edit-diary-entry', { entry: {{ $entry->id }} })" 
                ...
                <button type="button" 
                        wire:click="delete" 
                        wire:confirm="هل أنت تأكد من حذف هذه المذكرة؟" 
        ...
        <article class="rounded-2xl bg-surface-light dark:bg-surface-dark border border-gray-100 dark:border-gray-800 shadow-md overflow-hidden transition-all duration-300">
            ...
            <div class="h-1.5 bg-gradient-to-r from-primary via-emerald-400 to-teal-500"></div>
            ...
            <div class="p-6 sm:p-8 md:p-10 space-y-6">
                ...
                <header class="space-y-4 border-b border-gray-100 dark:border-gray-800/80 pb-6">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-primary/10 text-primary dark:bg-primary-dark/20 dark:text-primary-dark">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @if ($entry->mood)
                            <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-300 border border-amber-500/20">
                                <span>😊 المزاج:</span>
                    ...
                    @if (! empty($entry->tags))
                        <div class="flex flex-wrap gap-2 pt-1">
                            @foreach ($entry->tags as $t)
                                ...
                                <a href="{{ route('diary.index', ['tag' => $cleanTag]) }}" wire:navigate class="text-xs font-semibold px-3 py-1 rounded-full bg-primary/10 text-primary dark:bg-primary-dark/20 dark:text-primary-dark hover:bg-primary/20 transition">
                                    #{{ $cleanTag }}
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="prose dark:prose-invert max-w-none">
                    ...
                    @else
                        <p class="text-ink-soft dark:text-ink-dark-soft italic text-center py-10">
                            لا يوجد محتوى نصي لهذه المذكرة.
                        </p>
                    @endif
                </div>
            </div>
            ...
            <footer class="px-6 sm:px-10 py-4 bg-gray-50/50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800/60 flex items-center justify-between text-xs text-ink-soft dark:text-ink-dark-soft">
                <span>تاريخ الإنشاء: {{ $entry->created_at->translatedFormat('j M Y - g:i a') }}</span>
                @if ($entry->updated_at && $entry->updated_at->ne($entry->created_at))
                    <span>آخر تعديل: {{ $entry->updated_at->diffForHumans() }}</span>
                @endif
            </footer>
        </article>
    </div>
