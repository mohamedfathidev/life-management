<div class="py-8 min-h-screen">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="flex items-center justify-between gap-4">
            <a href="{{ route('recovery.topics') }}" wire:navigate class="inline-flex items-center gap-2 text-sm font-medium text-ink-soft hover:text-primary dark:text-ink-dark-soft dark:hover:text-primary-dark transition group">
                ...
                <span>تعلّم عن التعافي والإدمان</span>
            </a>

            <div class="flex items-center gap-2">
                <button type="button" 
                        wire:click="$dispatch('edit-topic', { topic: {{ $topic->id }} })" 
                ...
                <button type="button" 
                        wire:click="delete" 
                        wire:confirm="هل أنت تأكد من حذف هذا الموضوع؟" 
        ...
        <article class="rounded-2xl bg-surface-light dark:bg-surface-dark border border-gray-100 dark:border-gray-800 shadow-md overflow-hidden transition-all duration-300">
            ...
            <div class="p-6 sm:p-8 md:p-10 space-y-6">
                ...
                {{-- Header Title & Importance Badge --}}
                <header class="space-y-4 border-b border-gray-100 dark:border-gray-800/80 pb-6">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-{{ $topic->importance->color() }}/15 text-{{ $topic->importance->color() }} border border-{{ $topic->importance->color() }}/20">
                            <span>أهمية: {{ $topic->importance->label() }}</span>
                        </span>

                        <span class="text-xs text-ink-soft dark:text-ink-dark-soft">
                            {{ $topic->created_at->translatedFormat('j M Y') }}
                        </span>
                    </div>
                    ...
                    @if (! empty($topic->tags))
                        <div class="flex flex-wrap gap-2 pt-1">
                            @foreach ($topic->tags as $t)
                                ...
                                <a href="{{ route('recovery.topics', ['tag' => $cleanTag]) }}" wire:navigate class="text-xs font-semibold px-3 py-1 rounded-full bg-primary/10 text-primary dark:bg-primary-dark/20 dark:text-primary-dark hover:bg-primary/20 transition">
                                    #{{ $cleanTag }}
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="prose dark:prose-invert max-w-none">
                    ...
                    @else
                        <p class="text-ink-soft dark:text-ink-dark-soft italic text-center py-10">
                            لا يوجد محتوى نصي لهيك الموضوع.
                        </p>
                    @endif
                </div>
            </div>
            ...
            <footer class="px-6 sm:px-10 py-4 bg-gray-50/50 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800/60 flex items-center justify-between text-xs text-ink-soft dark:text-ink-dark-soft">
                <span>تاريخ الإنشاء: {{ $topic->created_at->translatedFormat('j M Y - g:i a') }}</span>
                @if ($topic->updated_at && $topic->updated_at->ne($topic->created_at))
                    <span>آخر تعديل: {{ $topic->updated_at->diffForHumans() }}</span>
                @endif
            </footer>
        </article>
    </div>
